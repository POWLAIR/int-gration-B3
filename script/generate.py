...
def process_markdown_file(input_file, output_file, template):
    ...
    match = re.search(r'(\d{4}-\d{2}-\d{2}-(.+))\.md$', os.path.basename(input_file))
    event_identifier = match.group(2) if match else 'event'
    image_filename = f'{event_identifier}.webp'
    image_path = os.path.join('..', 'ressource', image_filename)
...
def process_csv_file(input_file, output_file, template):
    ...
    rendered_html = template.render(members=members, base_path="..")
...
def main(input_folder):
    ...
    # Générer la page d'index dans dist
    index_output = os.path.join(output_folder, 'index.html')
    with open(index_output, 'w', encoding='utf-8') as f:
        f.write(index_template.render(articles=articles, base_path="."))
    
    print("\n✅ Generated index page in 'dist' folder")
